...
function composeFaviconGetterUri(target, size) {
...
function generateDynamicJS() {
    ...
    const searchEngineSwitchMenuJS = `<script defer="">
 ...
 **/
function generateStaticHTML(lang) {
    return `
      <!DOCTYPE html>
      <html lang="${lang}">
 ...
 */
async function generateDynamicHead(lang) {
    const headList = [
        "<head>",
 ...
 **/
function renderDynamicDiv1(lang) {
    const item = (template, name) => element("button", ['class="item"', `data-url="${template}"`], name);
    var headerHitoko = element("div", ['class="hitoko"'], element("p", [
        'id="hitokoto"',
        "onclick=\"location.href='https://hitokoto.cn';\"",
        'class="prevent-select"',
    ], "御剑于心，且听风吟。") + element("script", [], `fetch('https://v1.hitokoto.cn')
    ...
    .catch(console.error)`));
    var title = element("h1", ['class="title-div prevent-select"'], element("i", ['class="globe-icon"'], element("svg", [
        ...
        element("div", ['class="title"'], multiLang(lang, config.title) + element("div", ['class="sub-title"'], multiLang(lang, config.subtitle))));
    var searchEngineInput = element("div", ['class="search-engine-container"'], 
    // element(
    //   "div",
    ...
    element("img", [
        ...
        element("label", ['for="searchinput"'], "waht do you want to search") +
        element("input", [
            'id="searchinput"',
            'name="searchinput"',
            'type="search"',
        ...
        element("i", ['id="search"', 'class="search-engine-button"'], element("svg", [
            'xmlns="http://www.w3.org/2000/svg"',
            'width="1em"',
            'height="1em"',
            'fill="currentColor"',
    ...
    var searchEngineSwitchMenu = element("div", ['id="sengine"', 'class="search-engine-switch-menu prevent-select"'], config.search_engine
        .map((link, key) => {
        if (key == 0) {
            return element("button", ['class="active item"', `data-url="${link.template}"`], multiLang(lang, link.name));
        }
        else {
        ...
        .join(""));
    return element("header", ['class="header"'], (config.hitokoto ? headerHitoko : "") +
        element("div", ['id="head"', 'class="head-container"'], title)) + element("div", ['id="title"', 'class="search-engine-wrapper"'], (config.search ? searchEngineInput + searchEngineSwitchMenu : ""));
}
/**
 * 渲染第二个动态 div 的函数
 * @returns {string} 以字符串返回的第二个Div
 **/
function renderDynamicDiv2(lang) {
    var main = config.quickLinkLists
        .map((item) => {
        const card = (url, name, desc, icon_size) => element("a", ['class="card"', `href=${url}`, 'target="_blank"', 'rel="noreferrer noopener"'], element("div", ['class="content"'], element("img", [
        ...
        const divider = element("h2", ['class="horizontal-divider"'], element("i", ['class="quicklink-icon"'], item.icon) + multiLang(lang, item.name));
        var content = element("div", ['class="four-stackable-cards"'], item.quickLinkList
            .map((link) => {
    ...
    return element("div", ['class="scrollable"'], element("main", ['class="main"'], element("div", ['class="quicklink-div"'], main)));
}
/**
 * 渲染第三个动态 div 的函数
...
function renderDynamicDiv3() {
    return element("footer", ['class="footer"'], element("div", ['class="footer-div"'], "Powered by " +
        element("a", [
            'class="label"',
            'href="https://github.com/AHA1GE/cf-pages-index"',
            'target="_blank"',
            'rel="noreferrer noopener"',
        ], element("i", ['class="github-icon"'], element("svg", [
            'xmlns="http://www.w3.org/2000/svg"',
            'width="1em"',
            'height="1em"',
            'fill="currentColor"',
 ...
 **/
async function renderHTML(lang) {
    const staticHTML = generateStaticHTML(lang);
    const dynamicHead = await generateDynamicHead(lang);
    const dynamicDiv1 = renderDynamicDiv1(lang);
    const dynamicDiv2 = renderDynamicDiv2(lang);
